class flatModeButton {
	...
	static createButton(stereoImg) {
		...
		const controlslist = stereoImg.getAttribute('controlslist') || 'vr wiggle static anaglyph';
		const availableModes = ['static', 'wiggle', 'anaglyph'].filter(mode => controlslist.includes(mode));
		...
		container.onmouseenter = () => container.style.opacity = '1.0';
		...
		for (const mode of availableModes) {
			...
			button.onclick = () => stereoImg.setAttribute('flat', mode);
		...
		function updateHighlight() {
			const currentMode = stereoImg.getAttribute('flat') || 'wiggle';
